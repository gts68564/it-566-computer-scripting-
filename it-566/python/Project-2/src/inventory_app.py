...
class InventoryApp:
	...
	def process_menu_choice(self):
		...
		match self.menu_choice:
			...
			case self.EXIT:
				...
				if __debug__:
					print('It was Great Working With You!')
	...
	def search_items(self):
		...
		if choice == 1:
			...
			item = self.business_logic.search_items_by_id(item_id=item_id)
			self.print_search_item(item)
			input('Enter any key to continue...')
		elif choice == 2:
			item_name = str(input('Enter the name of the item:'))
			item = self.business_logic.search_items_by_name(item_name=item_name)
			self.print_search_item(item)
			input('Enter any key to continue...')
		else:
	...
	def print_search_item(self, item):
		t = PrettyTable(['ID', 'Items', 'Count', 'Inventory ID'])
		for row in item:
			t.add_row([row[0], row[1], row[2], row[3]])
		print(t)
